...
namespace L_Khant_000
{
    public partial class frmOriginal : Form
    {
        ...
        private void button5_Click(object sender, EventArgs e)
        {
            ...
            for (int i = 1; i < dgvOriginal.Columns.Count + 1; i++)
            {
                worksheet.Cells[1, i] = dgvOriginal.Columns[i - 1].HeaderText;
            }
            // storing Each row and column value to excel sheet  
            for (int i = 0; i < dgvOriginal.Rows.Count - 1; i++)
            {
                for (int j = 0; j < dgvOriginal.Columns.Count; j++)
                {
                    worksheet.Cells[i + 2, j + 1] = dgvOriginal.Rows[i].Cells[j].Value.ToString();
                }
            }
            // save the application  
            worksheet.SaveAs("c:\\output.xls");
